...
Uint32 netaction(Uint32 interval, void *param)//如果欄杆移動過程碰到皮卡丘 皮卡丘沒動的話就會有bug
{
...
Uint32 killball(Uint32 interval, void *param)
{
	...
	{
		ball_alpha = ball_alpha - 40;
		printf("殺球成功\n");

	}
	if (ball_alpha <= 40 || kill_ball_flag == false)
	{
		ball_alpha = 255;
...
Uint32 ballaction(Uint32 interval, void *param)
{
	...
	*/
	...
	{	//if (bally <= 0)
		...
		if (ballx + 96 / 2 == netx + 23 && bally + 96 >= 400)
		{
			up = 1;
			net = true;
		...
		if (up == 1 || left == 1 || right == 1 || down == 1)//球碰到皮卡丘
		{
			...
			if (kill_ball_flag == true && kill_ball_open == false)//一隻皮卡丘殺球後另一隻皮卡丘接到
			{
				//kill_ball_flag = false;
				//kill_ball_open = false;
			}
		...
		else//球碰到皮卡丘 球在天上or地面
		{
			...
			if (pika1_jump_flag == true)
			{
				if (bally + 96 + 50 >= pika1_jump_y && ((ballx + 96 + 30 >= pika1_jump_X && ballx + 96 - 30 <= pika1_jump_X + 120) || (ballx - 30 <= pika1_jump_X + 120 && ballx + 30 >= pika1_jump_X)) && bally_a > 0)/*碰到皮卡丘上界*/
				{
					printf("up");
			...
			{
				printf("up");
				if (bally + 96 + 50 >= 460 && ((ballx + 96 + 30 >= pika1_X && ballx + 96 - 30 <= pika1_X + 120) || (ballx - 30 <= pika1_X + 120 && ballx + 30 >= pika1_X)) && bally_a > 0)/*碰到皮卡丘上界*/
				{
					/*
					if (kill_ball_flag == true)//一隻皮卡丘殺球後另一隻皮卡丘接到
					{
			...
			if (pika2_jump_flag == true)
			{
				if (bally + 96 + 50 >= pika2_jump_y && ((ballx + 96 + 30 >= pika2_jump_X && ballx + 96 - 30 <= pika2_jump_X + 120) || (ballx - 30 <= pika2_jump_X + 120 && ballx + 30 >= pika2_jump_X)) && bally_a > 0)/*碰到皮卡丘上界*/
				{
					printf("up");
			...
			else
			{
				...
				if (bally + 96 + 50 >= 460 && ((ballx + 96 + 30 >= pika2_X && ballx + 96 - 30 <= pika2_X + 120) || (ballx - 30 <= pika2_X + 120 && ballx + 30 >= pika2_X)) && bally_a > 0)/*碰到皮卡丘上界*/
				{
					printf("up");
...
bool loadAudio()
{
	...
	effect1 = Mix_LoadWAV("../audio/jump12.mp3");
	if (effect1 == NULL)
	{
...
int main(int argc, char* args[])
{
	...
	char imgPath3[100] = "../images/beach.png";
	char beachPath[100] = "../images/beachselect.png";
	char classPath[100] = "../images/classic.png";
	...
	ImageData net, bg, bg2, classic, beach, ball, col, ball2, hole, prop, die;
	...
	bg2 = loadTexture(imgPath3, false, 0xFF, 0xFF, 0xFF);
	classic = loadTexture(classPath, false, 0xFF, 0xFF, 0xFF);
	beach = loadTexture(beachPath, false, 0xFF, 0xFF, 0xFF);
	...
	int select_character_left = 0, select_character_right = 0, select_background = 0;
	...
	while (!quit)
	{
		if (start == true)
		{
			...
			case IN_LB_SC:  // Inside, Left Button, Single Click
				...
				if (mouseX > 200 && mouseX < 200 + 158 && mouseY>HEIGHT - 300 && mouseY < HEIGHT - 300 + 140)
					select_background = (select_background + 1) % 2;
			...
			if (select_background == 0)
				imgRender(renderer, classic, 200, HEIGHT - 300);
			else
				imgRender(renderer, beach, 200, HEIGHT - 300);
